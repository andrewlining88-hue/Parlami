...
useEffect(()=>{
  ...
  fetch("/api/db",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"list",data:{}})})
    .then(r=>r.json()).then(d=>{
      ...
      const sorted=[...students].sort((a,b)=>((b.lifetimeMsgCount!=null?b.lifetimeMsgCount:b.totalMsgCount)||0)-((a.lifetimeMsgCount!=null?a.lifetimeMsgCount:a.totalMsgCount)||0));
  ...
  const generate = async () => {
    ...
    try {
      ...
      const r=await callClaude([{role:"user",content:"STUDENT LEVEL: "+studentLevel+". "+teacherBlock+" "+chatBlock+" "+mistakesBlock+" "+seed}],
        ...
        " Return ONLY a JSON array of exactly 6 exercises. 3 fill-in-blank with word bank: {type:fill,s:sentence with ___,a:correct,wb:[correct,wrong1,wrong2,wrong3],why:brief explanation}. 3 multiple choice: {type:mc,q:question,o:[opt1,opt2,opt3,opt4],a:correct,why:brief explanation}. CRITICAL — LANGUAGE RULE: every single word inside every entry in 'wb' and 'o' must be 100% Italian. Not one English word may appear anywhere inside an option, even mixed into an otherwise-Italian sentence, even as a near-homophone distractor. WRONG EXAMPLE (do not do this): \"Lei mangia been.\" — \"been\" is English, this is forbidden even though the rest of the sentence is Italian. CORRECT alternative: use a real or plausible-but-wrong ITALIAN spelling instead, like \"Lei mangia beno.\" This applies no matter what language the question ('q') itself is phrased in. The 'a' field must be copied EXACTLY character-for-character from one of the entries in 'wb'/'o' — same capitalization, same punctuation, same spacing. Do not rephrase or reformat it differently. The 'why' field is one short sentence explaining the grammar rule or reasoning behind the correct answer — written in "+(studentLevel==="A1"||studentLevel==="A2"?"simple English":"Italian, with the key rule in [it]...[/it] tags")+". No other text.");
      const start=r.indexOf("[");const end=r.lastIndexOf("]");
...
const [totalMsgCount,setTotalMsgCount]=useState(0);const [lifetimeMsgCount,setLifetimeMsgCount]=useState(0);const [recurringMistakes,setRecurringMistakes]=useState([]);const [tipLog,setTipLog]=useState([]);const [dismissedTip,setDismissedTip]=useState(null);const [savedWords,setSavedWords]=useState([]);const [todaysWords,setTodaysWords]=useState([]);const [studentReport,setStudentReport]=useState(null);const [loadingStudentReport,setLoadingStudentReport]=useState(false);const [categorizedVocab,setCategorizedVocab]=useState({});
...
useEffect(()=>{
  ...
  (async()=>{
    ...
    const menuOpts=[];
    if(hasNotes)menuOpts.push(beginner?"📚 [it]Rivedi la lezione con Andrei[/it] (Review your lesson with Andrei)":"📚 [it]Rivedi la lezione con Andrei[/it]");
    if(hasVocab)menuOpts.push(beginner?"📖 [it]Rivedi il vocabolario[/it] (Review vocabulary from your lesson)":"📖 [it]Rivedi il vocabolario della lezione[/it]");
    if(beginner){
      menuOpts.push("🎭 [it]Gioco di ruolo[/it] (Role play — restaurant, shopping, travel, doctor, airport)","🔄 [it]Pratica dei verbi[/it] (Verb practice)","💬 [it]Chiacchierata libera[/it] (Free chat — talk about anything)","📝 [it]Esercizi di grammatica[/it] (Grammar drills)");
    }else{
      menuOpts.push("🎭 [it]Gioco di ruolo (ristorante, shopping, viaggio, dottore, aeroporto)[/it]","🔄 [it]Pratica dei verbi[/it]","💬 [it]Chiacchierata libera[/it]","📝 [it]Esercizi di grammatica[/it]");
    }
    const menuText=menuOpts.map((o,i)=>(i+1)+") "+o).join("\n");
    const greeting="Ciao "+name.split(" ")[0]+"! 👋 "+(beginner?"[it]Cosa vuoi fare oggi?[/it] (What would you like to practice today?)":"[it]Come stai oggi? Cosa vuoi fare?[/it]")+"\n\n"+menuText;
...
useEffect(()=>{
  if(view!=="student"||!("serviceWorker" in navigator)||!("PushManager" in window))return;
  navigator.serviceWorker.ready.then(reg=>reg.pushManager.getSubscription()).then(sub=>setDeviceSubscribed(!!sub)).catch(()=>{});
},[view]);
  ...
  try{
    ...
    if(deviceSubscribed){
      ...
      if(existing){
        ...
        setPushSubscriptions(p=>p.filter(s=>s.endpoint!==endpoint));
      }
    ...
    setPushSubscriptions(p=>p.some(s=>s.endpoint===subJson.endpoint)?p:[...p,subJson]);
...
const menuMap="MENU SHORTCUTS: The activity menu shown to the student is:\n"+menuText+"\nIf the student's message is JUST a number matching one of these options (e.g. \"2\"), immediately switch to that exact activity, even if you were in the middle of something else like a role play. Abandon the previous topic completely and start the new activity right away — do not ask for confirmation, do not continue the old scene.";
const sys="You are Dante, an AI Italian tutor"+(isPreply?" created by Andrei, a professional Italian teacher":"")+". Warm, patient, passionate about Italian. "+(isPreply?"Help "+name+" practice between lessons with Andrei.":"Help "+name+" practice and improve their Italian.")+" Level: "+LN(level)+" ("+level+"). "+(studentGoal?"Goal: "+studentGoal+". ":"")+"THE MOST IMPORTANT RULES — FOLLOW THESE ABOVE ALL ELSE: 1) NEVER repeat a question or topic you already covered. Track everything you asked. 2) Keep responses SHORT: A1=1-2 sentences. A2=2-3. B1/B2=3-4. C1/C2=4-5. 3) After 5-6 exchanges on one topic, ask: want to continue or try something different? 4) End each message with ONE question that moves forward. 5) Correct max ONE mistake per message, briefly. GREETING RULE: The student has ALREADY been greeted with the activity menu today. Do NOT greet again. Do NOT show the menu again unless the student asks to change activity. Just respond directly to what they said. "+menuMap+" "+np+" "+vp+" "+mp+" ACTIVITIES: Follow student choice. Rivedi la lezione — briefly recap the topic from LESSON NOTES, then keep practicing it conversationally. Stay strictly within that topic; do not wander to unrelated subjects. Rivedi il vocabolario — review ONLY the exact words listed in LESSON VOCABULARY, one at a time. Never introduce other words instead. Give an Italian example sentence, definition, or context clue and have the student guess or use the word — do NOT switch into an English-prompt/Italian-answer translation-quiz format; this activity must still follow the LANGUAGE rule below like everything else. Role play — set scene, stay in character, switch scenario after 4-5 exchanges. Verb practice — conversational not drill-like, mix tenses. Grammar — one point at a time, through conversation not lists. Free chat — suggest 2-3 fresh topics suited to level. WHEN STUDENT SEEMS BORED (short 1-3 word answers 3+ times): switch to something fun. Do not push harder. CORRECTIONS: Correct the most important mistake briefly. Example: Just a small note — in Italian we say [it]sono andato[/it] not [it]ho andato[/it] because andare uses essere. Then move on. Let very minor errors slide. CULTURAL INSIGHTS: Once every 5-6 exchanges, if relevant to topic, share one brief cultural fact. One sentence only. LANGUAGE: "+(level==="A1"?"Lead every sentence in simple Italian first, then give the English translation right after in parentheses. Never default to English-first — Italian always comes first, even if the sentence is very short.":level==="A2"?"Mostly Italian. Add an English translation in parentheses only for new or harder words/sentences — do not translate simple words you've already used before.":level==="B1"?"Almost entirely Italian. English only for grammar explanations when truly necessary.":"Entirely in Italian.")+" REGISTER: Always address the student with the informal \"tu\" form (tu, ti, il tuo, hai, sei, piace a te...) — NEVER the formal \"Lei\" (Le, Suo, Ha, È, Le piace...). This applies even at higher levels, unless the student specifically asks to practice formal/business Italian. FORMAT — NEVER BREAK: No markdown, no **, no bullets, no numbered lists. Wrap ALL Italian in [it]...[/it] tags. When listing options use separate lines with emoji. Never correct English loanwords (drink, computer, sport, bar, ok, wifi). Never ask their name.";
...
const newTotal=totalMsgCount+1;setTotalMsgCount(newTotal);setLifetimeMsgCount(p=>p+1);
...
const handleChangeLevel=async(e,newLevel)=>{const d=await load("student:"+e);if(d){d.level=newLevel;d.totalMsgCount=0;const tfa={...(d.testFailedAt||{})};delete tfa[newLevel];d.testFailedAt=tfa;const priorLevels=LEVELS.slice(0,LEVELS.indexOf(newLevel));const newTests=Array.from(new Set([...(d.testsPassed||[]),...priorLevels]));d.testsPassed=newTests;await store("student:"+e,d);setStudents(p=>p.map(s=>s.email===e?{...s,level:newLevel,totalMsgCount:0,testFailedAt:tfa,testsPassed:newTests}:s));setSel(s=>s?{...s,level:newLevel,totalMsgCount:0,testFailedAt:tfa,testsPassed:newTests}:s);}};
...
const completeOnboarding=async(goal,lvl,daily)=>{
  ...
  await store("student:"+email,{name,email,level:lvl,passwordHash:hashPw(pw),messages:[welcomeMsg],badges:[],streak:0,lastDate:null,testsPassed:[],testFailedAt:{},vocabCount:0,lessonNote:"",lessonVocab:"",recurringMistakes:[],tipLog:[],dailyGoal:daily,totalMsgCount:0,lifetimeMsgCount:0,savedWords:[],messageCount:0,progress:0,badgeCount:0,studentGoal:goal,trialStart:new Date().toISOString().slice(0,10),subscriptionStatus:"free",isPreplyStudent:false});
