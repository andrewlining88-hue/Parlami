...
const DarkStyle = ({dark}) => dark ? <style>{`
  ...
  .dark-app .text-gray-900 { color: #faf9f7 !important; }
  .dark-app .bg-gray-50 { background: #111827 !important; }
  .dark-app .bg-gray-100 { background: #1f2937 !important; }
  .dark-app input, .dark-app textarea { background: #374151 !important; color: #faf9f7 !important; border-color: #4b5563 !important; }
...
const BADGES = [
{id:"first_steps",name:"First Steps",icon:"🎯",req:1,type:"messages",desc:"Send your first message",tier:"bronze"},
{id:"chatterbox",name:"Chatterbox",icon:"💬",req:100,type:"messages",desc:"Send 100 messages",tier:"bronze"},
{id:"dedicated",name:"Dedicated",icon:"⭐",req:500,type:"messages",desc:"Send 500 messages",tier:"silver"},
{id:"legend",name:"Legend",icon:"👑",req:2000,type:"messages",desc:"Send 2,000 messages",tier:"gold"},
{id:"streak7",name:"7-Day Streak",icon:"🔥",req:7,type:"streak",desc:"Practice 7 days in a row",tier:"bronze"},
{id:"streak30",name:"30-Day Streak",icon:"🌟",req:30,type:"streak",desc:"Practice 30 days in a row",tier:"silver"},
{id:"streak90",name:"90-Day Streak",icon:"💎",req:90,type:"streak",desc:"Practice 90 days in a row",tier:"gold"},
{id:"explorer",name:"Explorer",icon:"📖",req:100,type:"vocab",desc:"Learn 100 Italian words",tier:"bronze"},
{id:"wordmaster",name:"Word Master",icon:"📚",req:500,type:"vocab",desc:"Learn 500 Italian words",tier:"silver"},
{id:"linguist",name:"Linguist",icon:"🧠",req:1000,type:"vocab",desc:"Learn 1,000 Italian words",tier:"gold"},
{id:"polyglot",name:"Polyglot",icon:"🌍",req:3,type:"tests",desc:"Pass 3 level tests",tier:"silver"},
{id:"maestro",name:"Maestro",icon:"🎓",req:5,type:"tests",desc:"Pass 5 level tests",tier:"gold"},
];
const TESTS = {
...
function TeacherDash({dark,setDark,students,onLogout,onRemove,onResetPw,onSaveNote,onSaveVocab,onSendMsg,onChangeLevel,onToggleFree}) {
...
<div className="flex items-center space-x-2"><DarkToggle dark={dark} setDark={setDark}/><button onClick={()=>window.open("/leaderboard","_blank")} className="flex items-center space-x-1.5 text-xs font-semibold px-2.5 py-1.5 rounded-lg" style={{background:"#C8102E",color:"white"}}>🏆</button><button onClick={onLogout} className="flex items-center space-x-1.5 text-xs text-gray-400 hover:text-gray-700"><LogOut className="w-3.5 h-3.5"/><span>Logout</span></button></div>
</div>
...
<div className="grid grid-cols-2 gap-2">{BADGES.map(b=>{const u=(sel.badges||[]).includes(b.id);const tierColors={bronze:"#cd7f32",silver:"#9ca3af",gold:"#eab308"};return(
<div key={b.id} className="rounded-xl p-3" style={{background:u?(dark?"#422006":"#fefce8"):(dark?"#1f2937":"#f9fafb"),border:u?"2px solid "+tierColors[b.tier]:"1.5px solid "+(dark?"#374151":"#f0f0f0")}}>
<div className="flex items-center space-x-2"><span className={"text-lg "+(u?"":"grayscale opacity-30")}>{b.icon}</span><p className={"text-xs font-semibold "+(u?"":"text-gray-400")} style={u?{color:tierColors[b.tier]}:{}}>{b.name}</p>{u&&<span className="ml-auto text-green-500 text-xs">✓</span>}</div>
</div>);})}</div>
</div>
)}
...
function ProgressTab({messages,studentLevel,practiceStreak,vocabularyCount,testsPassed,unlockedBadges,chartFilter,setChartFilter,activityLog,onShowTest,recurringMistakes,tipLog,testFailedAt,totalMsgCount,studentReport,onGenReport,loadingStudentReport,onTabChange,dark=false}) {
...
const lp=Math.min(Math.floor(totalMsgCount/LEVEL_REQ[studentLevel]*100),100);
...
<div className="rounded-2xl p-5 text-white" style={{background:"#1a1a2e"}}><div className="flex justify-between items-start mb-4"><div><p className="text-xs opacity-50 mb-1">Current level</p><p className="text-4xl font-bold">{studentLevel}</p><p className="text-xs opacity-50 mt-1">{LN(studentLevel)}</p></div><div className="text-right"><p className="text-xs opacity-50 mb-1">Progress</p><p className="text-2xl font-bold">{lp}%</p><p className="text-xs opacity-50 mt-1">{Math.max(0,LEVEL_REQ[studentLevel]-totalMsgCount)} msgs left</p></div></div><div className="w-full rounded-full h-1" style={{background:"rgba(255,255,255,0.15)"}}><div className="h-1 rounded-full" style={{width:lp+"%",background:color}}/></div></div>
...
<div className="flex items-center justify-between mb-3">
<p className="text-xs font-bold uppercase tracking-wide" style={{color:"#6366f1"}}>📋 Dante's Report</p>
...
<button onClick={onGenReport} disabled={loadingStudentReport} className="mt-3 w-full py-2 rounded-xl text-xs font-semibold text-indigo-400 border border-indigo-100 hover:bg-indigo-50 transition-colors">{loadingStudentReport?"⏳ Generating report...":"🔄 Refresh Report"}</button>
</div>
):(
...
<button onClick={onGenReport} disabled={loadingStudentReport} className="w-full py-2.5 rounded-xl text-sm font-semibold text-white" style={{background:"#6366f1"}}>{loadingStudentReport?"⏳ Generating your report...":"Get My Report from Dante"}</button>
</div>
)}
<div className={cx.card}><div className="flex items-center space-x-2 mb-3"><p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">💡 Tips from Dante</p>{tipLog.length>0&&<span className="text-xs bg-indigo-50 text-indigo-400 px-2 py-0.5 rounded-full">{tipLog.length}</span>}</div>{tipLog.length===0?<p className="text-xs text-gray-300 text-center py-3">Tips will appear here every 5 messages.</p>:(()=>{const [tipIdx,setTipIdx]=useState(tipLog.length-1);const t=tipLog[tipIdx];return(<div><div className="flex items-start space-x-2.5 px-3 py-2.5 rounded-xl" style={{background:dark?"#1e1b4b":"#f5f3ff"}}><Star className="w-3 h-3 mt-0.5 flex-shrink-0" style={{color:"#6366f1"}}/><div><p className="text-xs text-indigo-700 leading-relaxed">{t.text}</p><p className="text-xs text-indigo-300 mt-0.5">{t.date}</p></div></div><div className="flex items-center justify-between mt-2"><button onClick={()=>setTipIdx(p=>Math.max(0,p-1))} disabled={tipIdx===0} className="text-xs px-2 py-1 rounded-lg disabled:opacity-20" style={{color:"#6366f1"}}>← Prev</button><p className="text-xs text-gray-300">{tipIdx+1} of {tipLog.length}</p><button onClick={()=>setTipIdx(p=>Math.min(tipLog.length-1,p+1))} disabled={tipIdx===tipLog.length-1} className="text-xs px-2 py-1 rounded-lg disabled:opacity-20" style={{color:"#6366f1"}}>Next →</button></div></div>);})()}</div>
<div className={cx.card}><div className="flex items-center space-x-2 mb-3"><p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">🔁 Dante is watching</p>{recurringMistakes.length>0&&<span className="text-xs bg-orange-50 text-orange-400 px-2 py-0.5 rounded-full">{recurringMistakes.length}</span>}</div>{recurringMistakes.length===0?<p className="text-xs text-gray-300 text-center py-3">Dante will track recurring mistakes after a few sessions.</p>:(()=>{const [mIdx,setMIdx]=useState(0);const m=recurringMistakes[mIdx];return(<div><div className="flex items-start space-x-2.5 px-3 py-2.5 rounded-xl" style={{background:dark?"#431407":"#fff7ed"}}><span className="text-orange-300 mt-0.5">⚠️</span><p className="text-xs text-orange-700 leading-relaxed">{m}</p></div><div className="flex items-center justify-between mt-2"><button onClick={()=>setMIdx(p=>Math.max(0,p-1))} disabled={mIdx===0} className="text-xs px-2 py-1 rounded-lg disabled:opacity-20" style={{color:"#ea580c"}}>← Prev</button><p className="text-xs text-gray-300">{mIdx+1} of {recurringMistakes.length}</p><button onClick={()=>setMIdx(p=>Math.min(recurringMistakes.length-1,p+1))} disabled={mIdx===recurringMistakes.length-1} className="text-xs px-2 py-1 rounded-lg disabled:opacity-20" style={{color:"#ea580c"}}>Next →</button></div><p className="text-xs text-gray-300 pt-1">Dante will gently correct these when they come up.</p></div>);})()}</div>
<div className={cx.card}><div className="flex items-center space-x-2 mb-3"><p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">📝 Study Tip of the Week</p></div>{(()=>{const tips=[{it:"Per ricordare meglio le parole nuove, scrivile e contemporaneamente leggile ad alta voce.",en:"To remember new words better, write them down and read them out loud at the same time."},{it:"Non imparare solo le parole che trovi nel libro, impara anche quelle che senti in altre situazioni e che per te sono importanti.",en:"Don't just learn words from books — also learn the ones you hear in real life that matter to you."},{it:"Ogni sera scrivi in italiano cinque o sei cose che hai fatto durante la giornata.",en:"Every evening, write five or six things you did during the day in Italian."},{it:"Cerca di studiare bene solo le parole che puoi usare attivamente. Il resto basta solo capirlo.",en:"Focus on learning words you can actively use. The rest, just understand it."},{it:"Non è possibile tradurre letteralmente le preposizioni. Per ricordarle cerca di impararle insieme ad una parola o in una frase.",en:"You can't translate prepositions literally. To remember them, learn them inside a word or phrase."},{it:"Se classifichi le parole in ordine tematico, è più facile ricordarle. Scrivi tutte le parole relative ad un argomento (es. cibi, bevande...).",en:"If you group words by topic, they're easier to remember. Write all words related to a theme (e.g. food, drinks...)."},{it:"Impara poche cose al giorno! E impara qualcosa tutti i giorni! Cerca di organizzare ogni giorno il momento dell'italiano.",en:"Learn a little every day! Try to set up your daily 'Italian moment'."},{it:"Cerca sempre di arrivare da solo ad una regola grammaticale, perché così la ricordi meglio, ed imparala subito con un esempio.",en:"Always try to figure out a grammar rule by yourself — you'll remember it better. Then learn it with an example."},{it:"Visualizzare le parole aiuta a ricordarle meglio. Collega le parole a delle immagini, un movimento, un rumore o un colore.",en:"Visualising words helps you remember them. Connect words to images, movements, sounds or colours."},{it:"Per studiare scegli un posto tranquillo e piacevole. Non imparare più di 8 vocaboli alla volta, ma ripetili spesso.",en:"Choose a quiet, pleasant place to study. Don't learn more than 8 words at a time, but repeat them often."},{it:"Scrivi le parole nuove su un quaderno, con una frase come esempio. In un'altra pagina scrivi la traduzione. Ogni tanto rileggi la lista.",en:"Write new words in a notebook with an example sentence. On another page write the translation. Re-read the list regularly."}];const week=Math.floor(Date.now()/(1000*60*60*24*7));const tip=tips[week%tips.length];return(<div className="px-3 py-3 rounded-xl" style={{background:dark?"#1c1917":"#fefce8"}}><p className="text-xs leading-relaxed font-medium" style={{color:dark?"#fbbf24":"#854d0e"}}>{tip.it}</p><p className="text-xs leading-relaxed mt-2" style={{color:dark?"#a16207":"#a16207",opacity:0.7}}>{tip.en}</p></div>);})()}</div>
<div className={cx.card}><style>{`@keyframes badgeGlow{0%{box-shadow:0 0 0 0 rgba(234,179,8,0.5)}70%{box-shadow:0 0 12px 6px rgba(234,179,8,0)}100%{box-shadow:0 0 0 0 rgba(234,179,8,0)}} @keyframes badgePop{0%{transform:scale(1)}50%{transform:scale(1.15)}100%{transform:scale(1)}} .badge-new{animation:badgeGlow 2s ease-in-out 3,badgePop 0.5s ease-in-out 1}`}</style><div className={cx.row+" mb-3"}><p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">🏅 Badges</p><p className="text-xs text-gray-300">{unlockedBadges.length}/{BADGES.length}</p></div><div className="grid grid-cols-2 gap-2">{BADGES.map(b=>{const u=unlockedBadges.includes(b.id);const raw=b.type==="messages"?totalMsgCount:b.type==="streak"?practiceStreak:b.type==="tests"?testsPassed.length:vocabularyCount;const p=Math.min(raw/b.req*100,100);const tierColors={bronze:"#cd7f32",silver:"#9ca3af",gold:"#eab308"};const tierBg={bronze:"#fef3e2",silver:"#f8f9fa",gold:"#fefce8"};const tierBgDark={bronze:"#451a03",silver:"#1f2937",gold:"#422006"};const isNew=u&&raw>=b.req&&raw<b.req*1.1;return(<div key={b.id} className={"rounded-xl p-3 transition-all "+(isNew?"badge-new":"")} style={{background:u?(dark?tierBgDark[b.tier]:tierBg[b.tier]):(dark?"#111827":"#f9fafb"),border:u?"2px solid "+tierColors[b.tier]:"1.5px solid "+(dark?"#374151":"#f0f0f0")}}><div className="flex items-center space-x-2 mb-1.5"><span className={"text-xl "+(u?"":"grayscale opacity-30")}>{b.icon}</span><div className="flex-1 min-w-0"><p className={"text-xs font-semibold truncate "+(u?"":"text-gray-400")} style={u?{color:tierColors[b.tier]}:{}}>{b.name}</p><p className="text-xs text-gray-300" style={{fontSize:"9px",textTransform:"uppercase",letterSpacing:"0.5px"}}>{b.tier}</p></div>{u&&<span className="text-green-500 text-sm">✓</span>}</div><p className="text-xs text-gray-400 mb-1.5 leading-tight">{b.desc}</p>{!u&&<><div className="w-full rounded-full h-1.5" style={{background:dark?"#374151":"#e5e7eb"}}><div className="h-1.5 rounded-full transition-all" style={{width:p+"%",background:tierColors[b.tier]}}/></div><p className="text-xs mt-1" style={{color:"#9ca3af",fontSize:"10px"}}>{raw.toLocaleString()} / {b.req.toLocaleString()}</p></>}{u&&<p className="text-xs mt-1" style={{color:tierColors[b.tier],fontSize:"10px",fontWeight:600}}>🎉 Unlocked!</p>}</div>);})}</div></div>
</div>);}
...
const [view,setView]=useState(()=>{
  ...
  return "login";
});const [name,setName]=useState(()=>{try{return localStorage.getItem("parlami_name")||"";}catch{return "";}});const [email,setEmail]=useState(()=>{try{return localStorage.getItem("parlami_email")||"";}catch{return "";}});
...
const hasNotes=!!lessonNote;const hasVocab=!!lessonVocab;const isPreply=hasNotes||hasVocab;
const sessionMsgs=msgs.filter(m=>m.date===new Date().toISOString().slice(0,10));
const exchangeCount=Math.floor(sessionMsgs.length/2);
const menuOptions=[];
if(hasNotes)menuOptions.push("📚 Review your lesson with Andrei");
if(hasVocab)menuOptions.push("📖 Review vocabulary from your lesson");
menuOptions.push("🎭 Role play (restaurant, shopping, travel, doctor, airport)","🔄 Verb practice","💬 Free chat — talk about anything","📝 Grammar drills");
const menuText=menuOptions.map((o,i)=>(i+1)+") "+o).join(" ");
const np=lessonNote?"LESSON NOTES (from teacher Andrei): \""+lessonNote+"\". Use ONLY if the student chooses to review their lesson. Do NOT force this topic.":"";
const vp=lessonVocab?"LESSON VOCABULARY (from teacher Andrei): \""+lessonVocab+"\". Use ONLY if the student chooses vocabulary review. Do NOT force these words into other activities.":"";
...
const sys="You are Dante, an AI Italian tutor"+(isPreply?" created by Andrei, a professional Italian teacher":"")+". You are warm, encouraging, patient and passionate about Italian. "+(isPreply?"Your job is to help "+name+" practice Italian between their lessons with Andrei.":"Your job is to help "+name+" practice and improve their Italian.")+" Student level: "+LN(level)+" ("+level+"). "+(studentGoal?"Learning goal: "+studentGoal+". ":"")+" OPENING MESSAGE RULE: If this is the FIRST message in the conversation (no previous messages from you), greet the student warmly and ask: What would you like to practice today? Then list these options on SEPARATE LINES: "+menuText+" Wait for their choice before starting any activity. Do NOT start teaching immediately. "+np+" "+vp+" "+mp+" ACTIVITY RULES: Once the student picks an activity, stay on it. After about 5-6 exchanges on the SAME activity, naturally say something like 'Great work on this! Want to keep going, or try something different?' and briefly list the options again. If the student says they want to change, present the menu again. NEVER stay on the same narrow topic for more than 6-7 exchanges unless the student explicitly wants to continue. Current exchange count this session: "+exchangeCount+". ANTI-REPETITION: Keep mental track of every question you ask. NEVER repeat a question or prompt you already used in this conversation. If you catch yourself about to ask something similar, pick a completely different angle. Vary your encouragement — do not say the same praise twice. ROLE PLAY RULES: When doing role play, set the scene clearly, take on a character (waiter, shop assistant, doctor etc), and stay in character. Let the student drive the conversation. Switch scenarios after 4-5 exchanges. VERB PRACTICE: Make it conversational, not drill-like. Ask questions that require specific verb forms. Mix tenses appropriate to their level. GRAMMAR DRILLS: Focus on one grammar point at a time. Give a brief explanation, then practice through conversation, not lists. FREE CHAT RULES: When the student picks free chat, suggest 2-3 interesting conversation topics appropriate for their level. Never suggest a topic you already covered in a previous session. A1-A2: simple daily life topics (food, family, hobbies, your city, daily routine). B1-B2: opinions, experiences, Italian culture, travel stories, current interests. C1-C2: debate, abstract topics, Italian society, literature, news. MOOD AWARENESS: If the student gives very short answers (1-3 words) for 3 or more messages in a row, they might be bored or tired. Switch to something more fun and interactive — suggest a quick role play scenario, a would-you-rather question in Italian, or a fun challenge. Do NOT push harder on grammar when they seem disengaged. CULTURAL INSIGHTS: When a topic naturally connects to Italian culture, briefly share one interesting cultural insight. For example, if discussing food mention that Italians never drink cappuccino after 11am. If discussing greetings mention the cheek-kiss custom. Keep it to one sentence, make it feel natural, not like a textbook fact. Only do this once every 4-5 exchanges, never force it. CORRECTIONS: When the student makes a grammar, spelling, or vocabulary mistake, always correct the most important one. Do it briefly and in English if it helps clarity — for example: 'Just a small note — in Italian we say [it]sono andato[/it] not [it]ho andato[/it] because andare uses essere.' Then move on. Correct max one mistake per message. Let only very minor errors slide. Never list or number corrections. GRAMMAR EXPLANATIONS: When explaining a grammar rule, keep it short, clear and in English. One rule at a time. LANGUAGE: "+(level==="A1"?"Speak mostly in English. Introduce only 1-2 Italian words or phrases per message, always translated.":level==="A2"?"Mix English and Italian. Keep Italian phrases short and always translate them in brackets.":level==="B1"?"Speak mostly Italian. Use English only for grammar explanations.":"Speak entirely in Italian.")+" STRICT FORMAT RULES — NEVER BREAK: 1) NO markdown ever. No **, no *, no #, no bullet points, no numbered lists. When listing options, write them as separate lines with the emoji prefix. 2) Wrap ALL Italian words or phrases in [it]...[/it] tags. 3) Keep responses SHORT — A1=1-2 sentences plus any menu. A2=2-3 sentences. B1/B2=3-4 sentences. C1/C2=4-5 sentences. 4) Always end with one question or prompt that moves the activity forward. 5) Never correct English loanwords used in Italian (drink, cocktail, computer, smartphone, sport, bar, ok, wifi, stress). Never ask for their name.";
...
if(newTotal%5===0){
  ...
  try{
    const fb=await callClaude(
      ...
      "You are Dante, an Italian language AI assistant. The student's name is "+name+". Read the conversation and write ONE specific, friendly tip (max 2 sentences) addressed to the student by name about something they actually did — a mistake, a word used incorrectly, or a grammar point they struggled with. Be warm and encouraging, not clinical. Example: 'Nice try with \"ho andato\" — movimento verbs like andare love essere: \"sono andato\". Think of it as: movement = essere!' If no errors, give one useful tip from what they discussed. Never generic advice. Do NOT flag English loanwords used in Italian (drink, cocktail, computer, smartphone, internet, sport, bar, club, stress, ok, wifi) — these are normal Italian."
    );
...
const completeOnboarding=async(goal,lvl,daily)=>{
  ...
  await store("student:"+email,{name,email,level:lvl,passwordHash:hashPw(pw),messages:[welcomeMsg],badges:[],streak:0,lastDate:null,testsPassed:[],testFailedAt:{},vocabCount:0,lessonNote:"",lessonVocab:"",recurringMistakes:[],tipLog:[],dailyGoal:daily,totalMsgCount:0,savedWords:[],messageCount:0,progress:0,badgeCount:0,studentGoal:goal,trialStart:new Date().toISOString().slice(0,10),subscriptionStatus:"free",isPreplyStudent:false});
...
const steps=["Your Italian","Your goal","Daily practice","Promo code"];
return(
...
<div className="w-full max-w-xs">
  <div className="text-center mb-10">
    ...
    <p className="text-sm mt-1.5" style={{color:"#9ca3af"}}>{onboardStep<3?"3 quick questions and you're in.":"Almost there!"}</p>
    <div className="flex justify-center gap-1.5 mt-5">
      {[0,1,2,3].map(i=><div key={i} className="h-0.5 rounded-full transition-all duration-300" style={{width:onboardStep>i?32:onboardStep===i?32:16,background:onboardStep>=i?"#1a1a2e":"#e5e7eb"}}/>)}
    </div>
  ...
  <div>
    ...
    <div className="grid grid-cols-2 gap-2">
      {DAILY.map(d=>(
        <button key={d.v} onClick={()=>{setDailyGoal(d.v);setOnboardStep(3);}} className="py-5 rounded-2xl text-center transition-all" style={{background:"white",border:"1.5px solid #f0f0f0",boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
  ...
  {onboardStep===3&&(
  <div className="text-center">
    <p className="text-sm mb-4" style={{color:"#6b7280"}}>Are you studying with Andrei?</p>
    <p className="text-xs mb-4" style={{color:"#9ca3af"}}>If your teacher gave you a promo code, enter it below.</p>
    <input value={promoCode} onChange={e=>setPromoCode(e.target.value)} placeholder="Enter promo code" className="w-full px-4 py-3 rounded-xl text-sm text-center border border-gray-200 mb-3 outline-none focus:border-gray-400" style={{background:"white"}}/>
    {promoCode==="PreplyParlami2026"&&<p className="text-xs font-semibold mb-3" style={{color:"#16a34a"}}>✨ Welcome! Your teacher will activate your VIP access.</p>}
    <button onClick={()=>completeOnboarding(studentGoal,level,dailyGoal)} className="w-full py-3 rounded-xl text-sm font-semibold text-white mb-2" style={{background:"#1a1a2e"}}>{promoCode?"Continue →":"Skip & start free trial →"}</button>
    <p className="text-xs" style={{color:"#d1d5db"}}>7 days free, then €6.99/month</p>
  </div>
  )}
</div>
</div>
...
if(view==="teacher") return <TeacherDash dark={dark} setDark={setDark} students={students} onLogout={()=>setView("login")} onRemove={handleRemove} onResetPw={handleResetPw} onSaveNote={handleSaveNote} onSaveVocab={handleSaveVocab} onSendMsg={handleSendMsg} onChangeLevel={handleChangeLevel} onToggleFree={handleToggleFree}/>;
...
<div className="bg-white rounded-2xl p-7 shadow-xl max-w-xs w-full mx-4 text-center">
...
{forgotSent?<><p className="text-sm text-green-600 font-medium mb-4">✓ Check your email!</p><button onClick={()=>setShowForgot(false)} className={cx.btn} style={{background:"#1a1a2e"}}>Done</button></>:<><input type="email" value={forgotEmail} onChange={e=>setForgotEmail(e.target.value)} placeholder="Your email" className={cx.input+" mb-3"}/><button onClick={async()=>{if(!forgotEmail.trim())return;await fetch("/api/send-reset",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:forgotEmail.trim().toLowerCase()})});setForgotSent(true);}} disabled={!forgotEmail.trim()} className={cx.btn+" mb-2"} style={{background:"#1a1a2e"}}>Send reset link</button><button onClick={()=>setShowForgot(false)} className="w-full text-xs text-gray-400 mt-1">Cancel</button></>}
</div>
</div>
)}
...
<PwInput value={tPw} onChange={e=>setTPw(e.target.value)} onEnter={()=>{if(tPw===TEACHER_PW){loadAll();setView("teacher");}}} placeholder="Teacher password" autoFocus/>
...
<div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 w-full max-w-sm text-center">
<div className="text-5xl mb-4">🇮🇹</div>
<h2 className="text-xl font-bold mb-2" style={{color:"#1a1a2e"}}>Your free trial has ended</h2>
<p className="text-sm text-gray-500 mb-6">Subscribe to keep practicing with Dante and access all features.</p>
<div className="rounded-xl p-4 mb-6" style={{background:"#faf9f7",border:"1px solid #f0f0f0"}}>
<p className="text-3xl font-bold" style={{color:"#1a1a2e"}}>€6.99<span className="text-sm font-normal text-gray-400">/month</span></p>
<p className="text-xs text-gray-400 mt-1">Unlimited conversations, vocab tracking, exercises & more</p>
</div>
<button onClick={async()=>{
  try{
    const r=await fetch("/api/create-checkout",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,name})});
    const d=await r.json();
    if(d.url)window.location.href=d.url;
    else alert("Something went wrong. Please try again.");
  }catch{alert("Could not connect to payment. Please try again.");}
}} className="w-full py-3 rounded-xl text-sm font-semibold text-white mb-3" style={{background:"#1a1a2e"}}>Subscribe now →</button>
<button onClick={logout} className="w-full py-3 rounded-xl text-sm text-gray-400 border border-gray-200">Log out</button>
</div>
</div>
);
...
<button onClick={()=>{const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){alert("Voice not supported. Use Chrome or Safari.");return;}const r=new SR();r.lang="it-IT";r.continuous=false;r.interimResults=false;r.onstart=()=>setListening(true);r.onend=()=>setListening(false);r.onresult=e=>{setCurMsg(p=>(p?p+" ":"")+e.results[0][0].transcript);};r.onerror=()=>setListening(false);r.start();}} disabled={typing} className="flex flex-col items-center justify-center disabled:opacity-40" title="Speak in Italian"><div className="w-11 h-11 rounded-xl flex items-center justify-center" style={{background:listening?"#ef4444":"#C8102E"}}><span style={{fontSize:"20px"}}>{listening?"⏹":"🎤"}</span></div><span className="text-xs font-medium mt-0.5" style={{color:listening?"#ef4444":"#C8102E",fontSize:"9px"}}>Speak</span></button><button onClick={send} disabled={typing||(!curMsg.trim()&&!file)} className="w-9 h-9 rounded-xl flex items-center justify-center text-white disabled:opacity-40" style={{background:"#1a1a2e"}}><Send className="w-4 h-4"/></button>
</div>
</div>
</div>
)}
